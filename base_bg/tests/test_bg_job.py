...
class TestBgJob(TransactionCase):
    ...
    def test_make_serializable(self):
        ...
        list_data = [partners, dt]
        result1 = base_bg.make_serializable(partners)
        result2 = base_bg.make_serializable(dt)
        result3 = base_bg.make_serializable(dict_data)
        result4 = base_bg.make_serializable(list_data)
        self.assertEqual(result1, partners.ids)
        self.assertEqual(result2, "2023-10-01T12:00:00")
        self.assertEqual(
            result3,
            {
        ...
        self.assertEqual(result4, [partners.ids, "2023-10-01T12:00:00"])

    def test_make_serializable_raises_on_unserializable(self):
        """make_serializable must raise ValueError for unserializable
        objects if filter_unserializable is False."""
        base_bg = self.env["base.bg"]
        ...
        with self.assertRaises(ValueError):
            base_bg.make_serializable(dict_data, filter_unserializable=False)
        with self.assertRaises(ValueError):
            base_bg.make_serializable(list_data, filter_unserializable=False)
        with self.assertRaises(ValueError):
            base_bg.make_serializable(function_data, filter_unserializable=False)

    def test_make_serializable_filters_unserializable(self):
        """make_serializable must omit unserializable values when filter_unserializable=True."""
        base_bg = self.env["base.bg"]
        # env is not serializable
        dict_data = {
            "serializable": "ok",
        ...
        list_data = ["ok", self.env, 123]
        result1 = base_bg.make_serializable(dict_data, filter_unserializable=True)
        result2 = base_bg.make_serializable(list_data, filter_unserializable=True)
        self.assertEqual(result1, {"serializable": "ok"})
        self.assertEqual(result2, ["ok", 123])
